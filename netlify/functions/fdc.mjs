...
export async function handler(event) {
  ...
  try {
    ...
    if (!process.env.FDC_API_KEY) return json({ error: "FDC_API_KEY fehlt" }, 500);
    ...
    else if (["n.b.","nb"].includes(unit)) { base = 0; baseType="g"; }
    ...
    const per100 = {
      kcal: pick(1008), protein: pick(1003), carbs: pick(1005),
      sugar: pick(2000), fat: pick(1004), satFat: pick(1258),
      fiber: pick(1079), sodium: pick(1093), cholesterol: pick(1253)
    };
    ...
    return json({ ok:true, source:{fdcId:food.fdcId,desc:food.description}, per100, amount:{value:amount,unit,approxWeight,base,baseType}, total });
  } catch(e) { return json({ error:String(e?.message|e) },500); }
}

function json(body, statusCode=200){return{statusCode,headers:{...corsHeaders,"content-type":"application/json"},body:JSON.stringify(body)}}
