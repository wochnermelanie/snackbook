 ...
 * /fdc Function
 * Query (GET/POST):
 * - name: Zutatentext (z.B. "Dinkelmehl")
 * - amount: Zahl der Menge
 * - unit: "g" | "ml" | "Stk"
 * - approxWeight: optional Gramm/ML je Stück (fällt zurück auf 60g)
 *
 * Antwort: Nährwerte (kcal, protein, carbs, sugar, fat, satFat, fiber, sodium, cholesterol)
 * Quelle: USDA FoodData Central (offizielle Daten)
 *
...
export async function handler(event) {
  ...
  try {
    ...
    // Menge in Gramm/ML normalisieren
    let base = 0;
    let baseType = "g";

    if (["g", "gramm"].includes(unit)) {
      base = amount;
      baseType = "g";
    } else if (unit === "ml") {
      base = amount;
      baseType = "ml";
    } else if (["stk", "stück", "st"].includes(unit)) {
      base = amount * (approxWeight || 60);
      baseType = "g";
    } else if (["n.b.", "nb", "n.b"].includes(unit)) {
      base = 0;
      baseType = "g";
    } else {
      base = amount;
    ...
    function pick(id) {
      const hit = (food.foodNutrients || []).find(
        n => n.nutrient?.number == id || n.nutrientId == id
      );
    ...
    // Faktor bestimmen (FDC liefert typ. je 100 g)
    let factor = 0;
