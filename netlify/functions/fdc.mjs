// USDA FoodData Central Proxy
// Erwartet: name, amount, unit, approxWeight
// Nutzt: process.env.FDC_API_KEY

const API = "https://api.nal.usda.gov/fdc/v1/foods/search";

const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Methods": "GET, POST, OPTIONS",
  "Access-Control-Allow-Headers": "Content-Type"
};

export async function handler(event) {
  if (event.httpMethod === "OPTIONS") {
    return { statusCode: 204, headers: corsHeaders };
  }

  try {
    const params = event.httpMethod === "GET"
      ? event.queryStringParameters || {}
      : JSON.parse(event.body || "{}");

    const name = (params.name || "").trim();
    const amount = Number(params.amount || 0);
    const unit = String(params.unit || "g").toLowerCase();
    const approxWeight = Number(params.approxWeight || 60);

    if (!name) return json({ error: "name fehlt" }, 400);
    if (!process.env.FDC_API_KEY) {
      return json({ error: "FDC_API_KEY fehlt (Netlify Env setzen)" }, 500);
    }

    // Menge in Gramm normalisieren
    let base = 0;
    let baseType = "g";
    if (["g", "gramm"].includes(unit)) { base = amount; baseType = "g"; }
    else if (unit === "ml") { base = amount; baseType = "ml"; }
    else if (["stk", "stück", "st", "ei", "eier"].includes(unit)) {
      base = amount * approxWeight;
      baseType = "g";
    } else { base = amount; baseType = "g"; }

    // FDC Suche
    const url = new URL(API);
    url.searchParams.set("api_key", process.env.FDC_API_KEY);
    url.searchParams.set("query", name);
    url.searchParams.set("pageSize", "1");
    url.searchParams.set("dataType", "Survey (FNDDS),Foundation,SR Legacy,Branded");

    const res = await fetch(url, { headers: { "Accept": "application/json" } });
    if (!res.ok) return json({ error: `FDC Fehler ${res.status}` }, 502);
    const data = await res.json();
    const food = (data.foods && data.foods[0]) || null;
    if (!food) return json({ error: `Keine FDC-Treffer für "${name}"` }, 404);

    // Mapping Nutrients
    const NMAP = {
      kcal: 1008, protein: 1003, fat: 1004, carbs: 1005,
      sugar: 2000, fiber: 1079, satFat: 1258,
      cholesterol: 1253, sodium: 1093
    };

    function pick(id) {
      const n = (food.foodNutrients || [])
        .find(n => n.nutrient?.number == id || n.nutrientId == id);
      return n ? Number(n.amount || 0) : 0;
    }

    const per100 = Object.fromEntries(Object.entries(NMAP).map(([k, id]) => [k, pick(id)]));

    const factor = baseType === "g" || baseType === "ml" ? base / 100 : 0;
    const total = Object.fromEntries(Object.entries(per100).map(([k, v]) => [k, round1(v * factor)]));

    return json({
      ok: true,
      source: { fdcId: food.fdcId, description: food.description, dataType: food.dataType },
      per100,
      amount: { value: amount, unit, approxWeight, base, baseType },
      total
    });
  } catch (e) {
    return json({ error: String(e?.message || e) }, 500);
  }
}

function json(body, statusCode = 200) {
  return { statusCode, headers: { "content-type": "application/json", ...corsHeaders }, body: JSON.stringify(body) };
}
function round1(n) { return Math.round((n + Number.EPSILON) * 10) / 10; }
